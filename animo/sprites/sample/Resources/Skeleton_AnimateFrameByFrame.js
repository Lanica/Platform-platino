...
game.addEventListener('onload', function(e) {
	...
	var text = platino.createTextSprite({text:'Touch left part of the screen for previous frame, right part for next frame.', fontSize:14});
...
function createSkeletonAtPosition(posX, posY)
{
        ...
        anim.setPaused(true);//we pause so that we can demonstrate changing frames one by one 
}
...
game.addEventListener('touchstart', function(e) {
    ...
    {
        var idx = -2;
        var animation = mySkeleton.getAnimation();
        if(animation)
        {
            if(e.x < game.screen.width/2)
            {
                idx =  animation.goToPreviousBonePositionFrame();
            }
            else{
                idx = animation.goToNextBonePositionFrame();
            }
        }
        
        Ti.API.info("new idx " + idx);
    }
});
