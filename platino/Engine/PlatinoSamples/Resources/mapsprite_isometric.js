...
var map = platino.createMapSprite({image:'graphics/isometric_grass_and_water.png', tileWidth:64, tileHeight:64, border:0, margin:0});
...
var mapfile = Ti.Filesystem.getFile(Titanium.Filesystem.resourcesDirectory, GRAPHICS_DIR + 'isometric_grass_and_water.json');
...
function updateVpad() {
    if (isVpadActive) {
        ...
        var maphalfX = map.width  * 0.5;
        var maphalfY = map.height;
        
        if (nextDogX > 0 && nextDogX < game.screen.width  - dog.width) {
            dog.x = nextDogX;
        } else if (nextMapX <= maphalfX && nextMapX > -maphalfX + game.screen.width){
            map.x = nextMapX;
        }
        if (nextDogY > 0 && nextDogY < game.screen.height - dog.height) {
            dog.y = nextDogY;
        } else if (nextMapY <= 0 && nextMapY > -maphalfY + game.screen.height){
            map.y = nextMapY;
        }
