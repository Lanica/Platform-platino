...
game.addEventListener('onload', function(e) {
	...
	var text = platino.createTextSprite({text:'Touch screen to play/stop animation. ', fontSize:14});
...
game.addEventListener('touchstart', function(e) {
    ...
    if(mySkeleton)
    {
        ...
        if(anim != null){
            mySkeleton.stopAnimation();//this will remove the animation entirely
        }
        else{
            //since we no longer have an animation - we need to create a new one
            var anim = animo.createSkeletalAnimationWithFile(animToLoadPath);
            mySkeleton.playAnimation(anim);
        }
    }
