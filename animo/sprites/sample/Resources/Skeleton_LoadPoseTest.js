...
game.addEventListener('onload', function(e) {
	...
	var text2 = platino.createTextSprite({text:'Touch screen to change skeleton pose', fontSize:14});
...
function createSkeletonAtPosition(posX, posY)
{
	...
	if(mySkeleton == null)
	{
		...
		var poseNames=new Array("DefaultPose","BowPose","DeathPose", "HatWave", "IdlePose", "PushPose", "ShootPose");
		
		var poseToLoadName = poseNames[Math.floor(Math.random() * poseNames.length)];
		
		mySkeleton.setPoseWithName(poseToLoadName);
	}
}
